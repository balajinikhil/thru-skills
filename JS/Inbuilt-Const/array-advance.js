...
for (let i = 0; i < inventors.length; i++) {
  let nm = inventors[i].first + " " + inventors[i].last;
  fullName.push(nm);
}
...
let fifteens = [];
for (let i = 0; i < inventors.length; i++) {
  if (inventors[i].year >= 1500 && inventors[i] <= 1600) {
...
//own
let f = inventors.MyFilter(function(ele, i, arr) {
  if (ele.year >= 1500 && ele.year <= 1600) {
    return true;
  } else {
    return false;
...
//reduce();
const totalYear = inventors.reduce(function(total, ele) {
